...
package com.mcxtzhang.layoutmanager.copy;
...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    public int checkHorizontalSwipe(ViewHolder viewHolder, int flags) {
        float abs = Math.abs(mRecyclerView.getWidth() / 2 - viewHolder.itemView.getX() - (viewHolder.itemView.getWidth() / 2));
        if (abs < 150) {
            return 0;
        }
            ...
            mValueAnimator.addUpdateListener(
                    new AnimatorUpdateListenerCompat() {
                        ...
                        public void onAnimationUpdate(ValueAnimatorCompat animation) {
                            ...
                            } else if (animatedFraction > 0.98f) {
                                animatedFraction += 0.03f;
                            } else if (animatedFraction > 0.90f) {
                                animatedFraction += 0.1f;
                            } else {

                            }
                            setFraction(animatedFraction);
                        }
                    });
